"""
Wipe and repopulate the employee table through the public API.
...
Deliberately drives the REST API rather than boto3 against DynamoDB. Seeding then
exercises the same validation, the same transactional create and the same checklist
handling the browser will, so a broken seed is a broken API rather than a mystery.
...
of the onboarding-system-dev CloudFormation stack via the AWS CLI.
"""
import argparse
import json
import os
...
def resolve_base_url(explicit):
    ...
    try:
        ...
        raise SystemExit('Could not read the stack output. Pass --base-url instead.\n' + str(error))

    if not output:
        raise SystemExit('Stack {} has no ApiBaseUrl output.'.format(STACK_NAME))
    return output.rstrip('/')


def wipe(base_url, assume_yes):
    employees = call(base_url, 'GET', '/employees')['employees']

    if not employees:
        print('Table is already empty. Nothing to wipe.')
        return

    print('\nAbout to DELETE {} employee(s):'.format(len(employees)))
    for employee in employees:
        print('  {}  {} {}'.format(employee['id'], employee['firstName'], employee['lastName']))

    if not assume_yes:
        # Destructive and irreversible - make someone type it.
        if input('\nType "delete" to confirm: ').strip().lower() != 'delete':
            raise SystemExit('Aborted.')

    for employee in employees:
        call(base_url, 'DELETE', '/employees/' + employee['id'])
        print('  deleted {}'.format(employee['id']))
    print('Wiped {} employee(s).'.format(len(employees)))
...
def main():
    ...
    parser.add_argument('--wipe', action='store_true', help='delete every existing employee first')
